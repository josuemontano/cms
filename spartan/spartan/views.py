...
from .factories import core
from .factories import extra
from .factories import security
from .factories import custom


@forbidden_view_config(renderer='403.html')
def forbidden(request):
    if custom.authenticated_userid(request):
        return { 'user' : custom.authenticated_userid(request) }
    else:
        next = request.route_url('login', _query=(('next', request.path), ))
        return custom.HTTPFound(location=next)


@notfound_view_config(append_slash=True, renderer='404.html')
...
class HomeView(object):
    def __init__(self, request):
        ...
        self.factory = extra.HomeFactory(request)
...
class UserViews(object):    
    def __init__(self, request):
        self.request = request
        self.factory = security.UserFactory(request)

    @view_config(route_name='login', renderer='login.html')
    def login(self):
        request = self.request
        if custom.authenticated_userid(request) is None:
...
class PagesViews(object):    
    def __init__(self, request):
        self.request = request
        self.factory = core.PageFactory(request)
...
class SiteViews(object):
    def __init__(self, request):
        self.request = request
        self.factory = core.SiteFactory(request)
...
class FilesViews(object):
    def __init__(self, request):
        self.request = request
        self.factory = extra.FileFactory(request)
...
class AccountsViews(object):
    def __init__(self, request):
        self.request = request
        self.factory = security.AccountFactory(request)
